...
// Create question. Admin only. Supports QuestionDTO { id?, content, type, answers: [{id?, content}] }
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    try { console.log("[api/quiz/create] incoming body:", JSON.stringify(bodyObj)); } catch {}
    if (!bodyObj?.content) return res.status(400).json({ error: "content_required" });
    // Normalize payload keys to match backend DTO
    const normalized = normalizeQuestionPayload(bodyObj);

    const { headers, attempts } = buildUpstreamAttempts(req, normalized, ["/question/create"]);
    let last = { status: 0, text: "", url: "", label: "" } as any;
    for (const a of attempts) {
      try {
        const r = await fetch(a.url, { method: "POST", headers: a.headers, body: a.body });
        const t = await r.text();
        console.log(`[api/quiz/create] upstream status=${r.status} label=${a.label}`);
...
function buildUpstreamAttempts(req: NextApiRequest, body: any, paths: string[]) {
  ...
  const variants: Array<{payload: any; label: string}> = [
    ...
    { payload: { id: body?.id, content: body?.content, type: body?.type, answers: body?.answers }, label: 'json_min' },
  ];
...
function devHeaders(res: NextApiResponse, headers: Record<string,string>, attempt: {label:string; url: string}, _req: NextApiRequest) {
