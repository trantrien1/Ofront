...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
      if (process.env.NODE_ENV !== 'production') {
        try { console.log('[post/delete] incoming body=', req.body); } catch {}
      }
    ...
    const url = `${upstream}/post/delete`;
      if (process.env.NODE_ENV !== 'production') {
        try { console.log('[post/delete] upstream url=', url); } catch {}
      }
    ...
    const txt = await r.text();
      if (process.env.NODE_ENV !== 'production') {
        try { console.log('[post/delete] upstream status=', r.status); } catch {}
      }
