...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    console.debug("/api/like proxy: incoming headers:", req.headers);
    console.debug("/api/like proxy: incoming body:", req.body);
    ...
    const r = await fetch(upstream, {
      method: req.method, // forward the same method (PUT/POST)
      headers,
      body: JSON.stringify(req.body),
    });

    const text = await r.text();
    console.debug(`/api/like proxy: upstream status=${r.status} body:`, text);
