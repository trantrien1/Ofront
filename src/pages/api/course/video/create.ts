...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const coureId = body?.coureId ?? body?.courseId ?? body?.id;
    if (!title) return res.status(400).json({ error: 'title_required' });
    if (!url) return res.status(400).json({ error: 'url_required' });
    if (coureId == null) return res.status(400).json({ error: 'coureId_required' });

    const payload = { title, description, url, coureId: Number(coureId) } as any;
    try { console.log('[api/course/video/create] payload ->', JSON.stringify(payload)); } catch {}

    const upstreamBase = process.env.UPSTREAM_URL || 'https://rehearten-production.up.railway.app';
    const urlUp = `${upstreamBase}/video/create`;
    const headers = buildHeaders(req);
    const r = await fetch(urlUp, { method: 'POST', headers, body: JSON.stringify(payload) });
    const t = await r.text();
    try { console.log(`[api/course/video/create] upstream status=${r.status} body_snippet=${(t||'').slice(0,500).replace(/\n/g,' ')}`); } catch {}
